...
function task710(data) {
  let firstAndLast = [posts[0], posts.at(-1)];
...
function task713(data) {
  let second = posts[2];
  console.log(
    `Post title is: '${second.title}' and the content is: '${second.body}'`
  );
}
task713(posts);
...
function task715(data) {
  let allTitle = posts.map((post) => post.title);
  console.log(allTitle);
}
task715(posts);
// 7.16. Išvesti visų masyvo narių property "body" į konsolę.
function task716(data) {
  let allTitle = posts.map((post) => post.body);
  console.log(allTitle);
}
task716(posts);
console.groupCollapsed("AllArray");
...
console.groupCollapsed("First Four");
...
console.groupCollapsed("Last seven");
function task719(data) {
  posts.slice(-7).forEach((post) => {
    console.log(`Title: '${post.title}' Content: '${post.body}'`);
  });
}
task719(posts);
...
function task722(data) {
  let result = posts.filter((post) => post.title.startsWith("s"));
...
function task724(data) {
  let result = posts.filter((post) => post.title.length > 15);
  console.log(result);
}
task724(posts);
...
function task725(data) {
  let result = posts.filter(
    (post) => post.title.length < 20 && post.body.length > 50
  );
  console.log(result);
}
task725(posts);
...
function task727(posts) {
  let result = posts.filter((post) => /\bsit\b/.test(post.body));
  console.log(result);
}
task727(posts);
